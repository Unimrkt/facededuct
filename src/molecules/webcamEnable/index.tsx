...
import QueryParamsExample from "../../king";
...
const WebcamDetection: React.FC = () => {
  ...
  const predictWebcam = async () => {
    if (!faceDetector) {
      ...
      try {
        ...
        // console.log(
        //   "🚀 ~ predictWebcam ~ detectionsResult:",
        //   faceDetector.detectForVideo(videoRef.current, performance.now())
        // );
  ...
  return (
    <div className="items-center justify-center space-y-6 p-6">
          ...
          return (
            boundingBox && (
              ...
              >
                ...
                {confidence && parseFloat(confidence) > 30 && (
                  <span
                    style={{
                      position: "relative",
                      top: "-20px",
      ...
      {detections.length === 0 && (
        <p className="text-gray-500">Kindly grant Permission to access the webcam by click on Enable Webcam, No faces detected yet.</p>
      )}
      <Router>
        <Routes>
          <Route path="/" element={<QueryParamsExample />} />
        </Routes>
      </Router>
    </div>
